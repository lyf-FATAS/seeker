...
##  📂  NVIDIA JETSO ONIN changes

Switch to the Jetson branch, git checkout jetson

If 8 distorted images need to be resolved, the following preparations are required:

```
The calibration file generated by Python 89generate_cali. py script 1
For example:
python 89generate_cali.py ~/catkin_ws/src/seeker1/config/seeker_omni_depth/kalibr_cam_chain.yaml

Then it will generate/tmp/cali and put it in the path~/catkinw_s/src/seeker1/config/seeker_omni_depth
```

Images can be cropped by modifying the code themselves:/resitfy corresponds from top to bottom as follows:
...
Run ros1 using 1seeker_nodelet.launch

Run ros2 using 1seeker.launch.cpy
